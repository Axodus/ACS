...
export function ProductUI({compact=false}:{compact?:boolean}){
 return <div className={`product-ui ${compact?'compact':''}`} aria-label="ACS Standalone App interface preview" role="img">
  ...
  <div className="app-shell">
   <aside><div className="app-logo"><span>A</span><b>ACS</b><small>CONTROL PLANE</small></div>{['Overview','Agents','Roles','Profiles','Skills','Plugins','Memory','Runtime','Logs'].map((x,i)=><div className={`side-item ${i===1?'active':''}`} key={x}><span>{['◇','⬡','⌁','◎','✦','⎔','◫','◉','≡'][i]}</span>{x}{i===1&&<em>3</em>}</div>)}<div className="workspace"><i>MB</i><span>local workspace<small>OpenClaw running</small></span></div></aside>
