...
export interface EventEnvelopeV2 {
  ...
  readonly subject_type?: "integration_connection" | "integration_channel" | "memory_policy" | "memory_record" | "delegation_grant";
...
export function validateEventEnvelopeV2(value: unknown): EventEnvelopeV2 {
  ...
  if (event.subject_type !== undefined && !["integration_connection", "integration_channel", "memory_policy", "memory_record", "delegation_grant"].includes(event.subject_type)) issues.push({ path: "subject_type", code: "INVALID_ENUM", message: "Invalid canonical subject type" });
