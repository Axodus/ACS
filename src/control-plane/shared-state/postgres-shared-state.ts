...
export class PostgresSharedAuthoritativeState implements SharedAuthoritativeState {
  ...
  constructor(options: PostgresSharedAuthoritativeStateOptions) {
    ...
    this.nativeCore = {
      ...
      listIntegrationConnectionDefinitions: (input) => session.nativeCore.listIntegrationConnectionDefinitions(input),
      ...
      listIntegrationChannelDefinitions: (input) => session.nativeCore.listIntegrationChannelDefinitions(input),
      ...
      createActivation: (input) => this.withTransaction("create causal Activation", (tx) => tx.nativeCore.createActivation(input)),
      claimActivation: (input) => this.withTransaction("claim causal Activation", (tx) => tx.nativeCore.claimActivation(input)),
      transitionActivation: (input) => this.withTransaction("transition causal Activation", (tx) => tx.nativeCore.transitionActivation(input)),
      prepareActivationHandoff: (input) => this.withTransaction("prepare Activation admission handoff", (tx) => tx.nativeCore.prepareActivationHandoff(input)),
      getActivationLineage: (activationId, tenantId) => this.withTransaction("read Activation lineage", (tx) => tx.nativeCore.getActivationLineage(activationId, tenantId)),
      saveScheduleRecoveryWatermark: (input) => this.withTransaction("save Activation schedule watermark", (tx) => tx.nativeCore.saveScheduleRecoveryWatermark(input)),
      getScheduleRecoveryWatermark: (input) => this.withTransaction("read Activation schedule watermark", (tx) => tx.nativeCore.getScheduleRecoveryWatermark(input)),
      ...
      listMemoryRecordStates: (input) => this.withTransaction("list Memory Record metadata", (tx) => tx.nativeCore.listMemoryRecordStates(input)),
