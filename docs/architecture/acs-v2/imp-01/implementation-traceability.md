...
| REQ-04 generic durable idempotency | `acs_native_idempotency`, `idempotent` | Scope/key/request hash bind one succeeded command result; a conflicting request is explicit and duplicate mutation is avoided. | VAL-01 PostgreSQL test. | Durable duplicate prevention passed, but different-hash conflict is wrapped as a generic `TransactionFailedError` at the public boundary. Gate remains failed. |
| REQ-04 fencing and checkpoints | `recordFencedCheckpoint`, `assertRuntimeOwnership`, `acs_native_checkpoints` | Native checkpoint/event/outbox writes require the existing durable assignment, lease, worker instance, service principal, and fencing token. | Existing `tests/s49-epic-15-5-durable-runtime-state.test.mjs`; VAL-01 PostgreSQL test. | Checkpoint persistence and stale-write prevention passed, but the public stale rejection is wrapped as generic transaction failure. Gate remains failed. |
