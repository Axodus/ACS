...
export function Readiness() {
  ...
  return <>
    ...
    <div className="dashboard-grid readiness-grid">
      ...
      <Shared.DashboardCard title="Readiness evidence" meta="Shared.Domain evidence used for status" state={cardState(summary?.evidence.length)} emptyMessage="No readiness evidence available">
...
export function CustomerDashboard() {
  ...
  return <div className="dashboard-canvas">
    ...
    <div className="dashboard-kpis" aria-label="Key operational indicators">
      <Shared.DashboardMetric label="Active Agents" value={summary?.agents.active ?? "--"} context={summary ? `${summary.agents.total} total agents` : "Loading current inventory"} action="Shared.View Agents" to="/agents" icon={<Icons.Robot size={25} weight="fill" />} accent="agents" progress={agentProgress} />
      <Shared.DashboardMetric label="Executions" value={summary?.executionRuns.total ?? "--"} context={summary ? `${summary.executionRuns.running} running now` : "Loading execution state"} action="Shared.View Executions" to="/executions" icon={<Icons.PlayCircle size={25} weight="fill" />} accent="executions" progress={executionProgress} />
      ...
      <Shared.DashboardMetric label="Usage" value={economicValue(economics.data?.totalMetered, economics.data?.unit)} context={economics.loadError ? "Financial data unavailable" : "Current operational metering"} action="Shared.View Financial Operations" to="/economics" icon={<Icons.CurrencyDollar size={25} weight="fill" />} accent="usage" progress={null} />
      <Shared.DashboardMetric label="Workers" value={summary?.workers.available ?? "--"} context={summary ? `${summary.workers.availableSlots} open slots` : "Loading worker capacity"} action="Shared.View Workers" to="/workers" icon={<Icons.HardDrives size={25} weight="fill" />} accent="workers" progress={workerProgress} />
    ...
    <div className="dashboard-cockpit-grid">
      <Shared.CockpitPanel title="Execution Activity" meta="Current execution lifecycle distribution" state={rootState} className="cockpit-execution" accent="executions" action={<Router.Link className="cockpit-action" to="/executions">Shared.View all<Icons.ArrowRight size={13} weight="bold" /></Router.Link>}>
      ...
      <Shared.CockpitPanel title="Service Health" meta="Customer-relevant service availability" state={rootState} className="cockpit-services" accent="success" action={<Router.Link className="cockpit-action" to="/operations">Shared.View all<Icons.ArrowRight size={13} weight="bold" /></Router.Link>}>
      ...
      <Shared.CockpitPanel title="Recent Activity" meta="Latest operational events" state={activity.loadState === "error" ? "error" : activity.loadState} className="cockpit-recent" accent="info" action={<Router.Link className="cockpit-action" to="/operational-evidence">Shared.View all activity<Icons.ArrowRight size={13} weight="bold" /></Router.Link>}>
        {events.length === 0 ? <div className="recent-empty"><Icons.ClockCountdown size={34} weight="duotone" /><b>No recent activity</b><span>Authoritative events will appear here as they occur.</span></div> : <div className="recent-activity-table" role="table" aria-label="Recent operational activity"><div className="recent-activity-head" role="row"><span>Shared.Time</span><span>Event</span><span>Detail</span><span>Shared.Status</span></div>{events.map(event => <article key={event.eventId} role="row"><time dateTime={new Date(event.createdAt).toISOString()}>{new Date(event.createdAt).toLocaleTimeString()}</time><b>{event.type}</b><span>{event.message}</span><Shared.FindingSeverity severity={event.severity === "critical" ? "error" : event.severity} /></article>)}</div>}
      </Shared.CockpitPanel>

      <Shared.CockpitPanel title="Quick Access" meta="Supported ACS workflows" state="ready" className="cockpit-quick" accent="info">
        <div className="quick-access visual"><Router.Link to="/agents/new"><span><Icons.Robot size={23} weight="duotone" /></span><b>Create Agent</b></Router.Link><Router.Link to="/operational-execution"><span><Icons.RocketLaunch size={23} weight="duotone" /></span><b>Plan Execution</b></Router.Link><a href={Api.productApiConfig.tenantAdministrationUrl}><span><Icons.UserPlus size={23} weight="duotone" /></span><b>Add Tenant</b></a><Router.Link to="/economics"><span><Icons.ChartBar size={23} weight="duotone" /></span><b>Shared.View Usage</b></Router.Link><Router.Link to="/economics"><span><Icons.Wallet size={23} weight="duotone" /></span><b>Reservations</b></Router.Link><Router.Link to="/economics"><span><Icons.CurrencyDollar size={23} weight="duotone" /></span><b>Settlements</b></Router.Link><Router.Link to="/administration"><span><Icons.ShieldCheck size={23} weight="duotone" /></span><b>Providers</b></Router.Link><Router.Link to="/operations"><span><Icons.Wrench size={23} weight="duotone" /></span><b>Operations</b></Router.Link></div>
      </Shared.CockpitPanel>
    </div>

    {(summary?.globalCaveats.length ?? 0) > 0 && <aside className="administration-advisory"><div><b>System configuration has advisory notices</b><span>Global certification caveats are tracked separately from current customer health.</span></div><Router.Link to="/administration">Shared.View Administration Overview →</Router.Link></aside>}
  </div>;
}
