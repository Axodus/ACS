...
const outputPath = process.argv[2] || "/tmp/acs-aees-16-03-browser-recovery/manifest.json";
...
const manifest = {
  schemaVersion: 1,
  certification: "AEES-16-03/ACCEPTANCE-RECOVERY-01",
...
async function runBrowserAcceptance(playwright, url) {
  ...
  manifest.noFakeData = {
    ...
    fabricatedHistory: checks.some((check) => check.fabricatedHistory) ? 1 : 0,
    usageSurfaceVisible: checks.every((check) => check.usageSurfaceVisible),
    settlementSurfaceVisible: checks.every((check) => check.settlementSurfaceVisible),
    receiptSurfaceVisible: checks.every((check) => check.receiptSurfaceVisible),
  };
  manifest.result =
    ...
    manifest.noFakeData.fabricatedHistory === 0 &&
    manifest.noFakeData.usageSurfaceVisible &&
    manifest.noFakeData.settlementSurfaceVisible &&
    manifest.noFakeData.receiptSurfaceVisible
      ? "PASS"
      : "FAIL";
}
